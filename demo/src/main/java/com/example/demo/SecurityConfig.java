...
//         .requestMatchers(mvc.pattern("/cloudinsider")).permitAll().anyRequest().authenticated())
...
//     }
http.authorizeHttpRequests(authz -> authz
     .requestMatchers(mvc.pattern("/cloudinsider"))
     .authenticated())
     .csrf(c -> c.disable())
     .logout((logout) -> logout.logoutSuccessHandler(oidcLogoutSuccessHandler()));
http.oauth2Login((oauth2Login) -> oauth2Login
.userInfoEndpoint(userInfo -> userInfo.userAuthoritiesMapper(grantedAuthoritiesMapper()))
.defaultSuccessUrl("http://localhost:8082/cloudinsider", true));
